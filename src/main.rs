...
use gabagool::{Interpreter, Store, Value, ValueType};
...
fn main() -> Result<()> {
    ...
    let mut interpreter = Interpreter::instantiate(&mut store, &wasm_bytes, vec![])?;
    ...
    let results = interpreter.invoke_export(&func_name, values)?;
