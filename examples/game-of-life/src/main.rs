...
use gabagool::{Interpreter, Value};
...
fn call_i32(interpreter: &mut Interpreter, name: &str) -> Result<i32> {
...
fn read_framebuf(interpreter: &Interpreter, ptr: usize, len: usize) -> &[u32] {
...
fn save_snapshot(interpreter: &Interpreter, path: &str) -> Result<()> {
...
fn fork_snapshot(interpreter: &Interpreter, fork_count: u32) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    let win_offset: i32 = args
        .iter()
        .position(|a| a == "--offset")
        .and_then(|i| args.get(i + 1)?.parse().ok())
        .unwrap_or(0);

    let mut interpreter = if let Some(path) = restore_path {
        let bytes =
            std::fs::read(path).with_context(|| format!("failed to read snapshot file: {path}"))?;
        let interpreter =
            Interpreter::from_snapshot(&bytes).context("failed to restore from snapshot")?;
        println!("Restored from {path}");
        interpreter
    } else {
        let wasm_bytes = include_bytes!("../wasm/game.wasm");
        let mut interpreter = Interpreter::new(wasm_bytes).context("failed to load wasm")?;
